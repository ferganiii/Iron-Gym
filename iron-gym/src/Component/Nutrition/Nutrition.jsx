...
export default function Nutrition() {
  ...
  return (
    ...
    {/* القسم الذي يحتوي على الخلفية والمحتوى العلوي */}
    <div
      className="h-screen bg-cover bg-center relative"
      style={{ backgroundImage: `url(${background})` }}
    >
      <div className="container mx-auto h-full flex flex-col items-center justify-center text-center">
        <h1 className="lg:text-8xl max-md:text-4xl max-sm:text-3xl text-white font-Antic">
          Nutrition
        </h1>
       
      </div>
      {/* الموجة تمتد على كامل عرض الخلفية */}
      <div className="absolute bottom-0 left-0 w-full">
        ...
        {/* العنوان داخل الموجة */}
        <h2 className="absolute bottom-10 left-1/2 transform -translate-x-1/2 -translate-y-10 text-white font-archivo lg:text-5xl max-md:text-4xl max-sm:text-3xl text-center">
          GUIDES AND PROGRAMS
        </h2>
      </div>
    </div>
  
    {/* باقي المحتوى أسفل الخلفية مع الـ container */}
    <div className="container mx-auto relative  ">
      <div className="mt-10 py-10">
        <h2 className="text-5xl font-archivo text-left ">
          NUTRITION GUIDE
        </h2>
      </div>
          ...
          {nutritionData.map((item) => (
            ...
            >
              <Link to={`/nutrition/${item.link}`}>
                <img src={item.image} alt={item.title} className="w-full h-80 hover:scale-105 transition-all duration-500 border border-gray-400 rounded-lg" />
              </Link>
              <div className="p-5">
                <Link to={`/nutrition/${item.link}`}>
