...
async function handelDlete(id) {
   
      try {
        await axios.delete(
          `https://gym-production-8217.up.railway.app/api/brosplit/${id}`,
        ...
        setSystems((prev) => prev.filter(system => system._id !== id));
      } catch (error) {
        console.log(error);
      }
    }
...
>
  <Link to='/CreateSystemModal'> + إضافة نظام تدريبي</Link>
</button>
              ...
              {systems.map((system, index) => (
                ...
                >
                  <h3 className="text-xl font-bold text-orange-500 dark:text-white">
                    <button className='bg-red-500 p-3' onClick={() => handelDlete(system._id)} >delet</button>
                    <span className="text-orange-500">{system.day} : </span>({system.exercises?.length || 0} Exercises)
                  </h3>
